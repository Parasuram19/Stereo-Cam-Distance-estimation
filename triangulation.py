...
def run():
    ...
    try:
        ...
        # cameras variables
        left_camera_source = "triangulation\\videos\SLOMO.mp4"
        right_camera_source = "triangulation\\videos\SLOMO.mp4"
        pixel_width = 1280 #640
        pixel_height = 720 #480
        angle_width = 66.9
        angle_height = 52.6 # 63
        frame_rate = 126
        # camera_separation = 16.5 #original
        camera_separation = 16.5                                                                                                                                                                                    
        # left camera 1
        ...
        # variables
        maxsd =100 # maximum size difference of targets, percent of frame #2
        klen  = 10 # length of target queues, positive target frames required to reset set X,Y,Z,D #3
        ...
        while 1:
            ...
            # canny1=cv2.Canny(frame1, 125,175)
            

            # dilated1=cv2.dilate(canny1,(7,7),iterations=3)
            # cv2.imshow("dilated1",dilated1) 
            # canny2=cv2.Canny(frame2, 125,175)
          

            # dilated2=cv2.dilate(canny2,(7,7),iterations=3)
            # cv2.imshow("dilated2",dilated2)
            # hsv = cv2.applyColorMap(frame1, cv2.COLORMAP_HSV)
            # cv2.imshow('HSV',hsv)  
            # hsv2 = cv2.applyColorMap(frame2, cv2.COLORMAP_HSV)
